...
int calc_area_of_sum_Rectangle(vector<Rectangle> all_rectangles){
	...
	Rectangle sumRect = Rectangle();
	int x;

	for (x = 0; x < number_of_rectangles; x++)
	{
		sumRect = sumRect + all_rectangles[x];
	}
...
int calc_perimeter_of_product_Rectangle(vector<Rectangle> all_rectangles){
	int number_of_rectangles = all_rectangles.size();
	//INSERT YOUR CODE HERE:
	Rectangle prodRect = Rectangle();
	int x;

	for (x = 0; x < number_of_rectangles; x++)
	{
		prodRect = prodRect * all_rectangles[x];
	}
	
	return prodRect.perimeter();
