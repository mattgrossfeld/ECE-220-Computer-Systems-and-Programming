...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(ptr->left == NULL && ptr->right == NULL)			// if left and right are NULL then it is leaf node
		return 1;
	return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(ptr->cutline == V || ptr->cutline == H)			// if the enum identifier is V or H then it is an internal node
		return 1;
	return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	...
	if( b != NULL)
	{
		if( b == a)										// base case return 1 if in subtree
			return 1;
		else
			return is_in_subtree(a,b->parent);			// return b-> parent to check upwards
	}
	return 0;
}
...
void rotate(node_t* ptr) {
  // TODO: Rotate the module, swapping the width and height.
	...
	int temp_w;
	int temp_h;
	temp_w = ptr->module->w;				// store the values temporarily
	temp_h = ptr->module->h;
	ptr->module->w = temp_h;				// switch the values
	ptr->module->h = temp_w;
//	printf("%d %d\n",temp_w, temp_h);
}
...
void recut(node_t* ptr) {
  ...
  // TODO: 
	...
	if(ptr->cutline == H)			// if Horizontal
	{
		ptr->cutline = V;			// switch to Vertical
		return;
	}
	if( ptr->cutline == V)			// if Vertical
	{
		ptr->cutline = H;			// switch to Horizontal
  		return;
	}
}
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
	module_t* temp_a;		// create some temporary variables to hold the pointers
	module_t* temp_b;
	
	temp_a = a->module;		// store the pointers
	temp_b = b->module;		

	a->module = temp_b;		// swap the pointers
	b->module = temp_a;
}
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
	...
	node_t* parentofA = a->parent; //get the parent of A
	node_t* parentofB = b->parent; //get the parent of B

	if (parentofB->right == b) //if nodeB is on the right of parentB, swap it with A.
	{
		parentofB->right = a;
	}
	...
	a->parent = parentofB; //swap parent of A with parent of B.

	if (parentofA->right == a) //if nodeA is on right of parentA, swap it with B.
	{
		parentofA->right = b;
	}
	...
	b->parent = parentofA; //swap parent of B with parent of A
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO: (remember to remove the following return statement)
	...
	if( par == NULL)												// = create the root node
	{
		par = (node_t*)malloc(sizeof(node_t));
		par->cutline = V;
		par->module = NULL;
		par->parent = NULL;
		init_slicing_tree(par,n+1);
	}
	par->right = (node_t*)malloc(sizeof(node_t));		// make space for leaf node
	par->left = (node_t*)malloc(sizeof(node_t));		// make space for internal nodes
	
	if(n <= (num_modules - 3) && par != NULL)				// num_modules-3 because of leaf and internal
	{	
		par->left->parent = par;
		par->left->left = init_slicing_tree(par->left,n+1);
		par->left->parent = par;
		par->left->cutline = V;
		par->left->module = NULL;

	}
	else
	{
		par->left->module = &(modules[n+1]);		// make the last internal node 
		par->left->left = NULL;
		par->left->right = NULL;
	...
	par->right->module = &(modules[n]);			// create the leaf nodes always the same
