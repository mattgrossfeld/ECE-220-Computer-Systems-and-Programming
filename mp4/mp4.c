/*
First we initialize our variable bound_on_roots as the value returned from our rootbound function. 
Then, if the bound is zero, there are no roots in the polynomial.
If there is a positive value for the bounds, then we use Halley's method to get the root and loop the function, incrementing the left bound by 0.5 each time.
If no roots are found, then it will state that no roots were found.
*/
...
double abs_double(double y)
{
	...
	double absolute_value = fabs(y); // get absolute value of y
...
double fx_val(double a, double b, double c, double d, double e, double x)
{
	...
	double y;
	y = a * (x*x*x*x) + b * (x*x*x) + c * (x*x) + d * x + e; // f(x) = ax^4 + bx^3 + cx^2 + dx + e
    return y;
}
...
double fx_dval(double a, double b, double c, double d, double e, double x)
{
	...
	double y_derivative;
	y_derivative = 4*a*(x*x*x) + 3*b*(x*x) + 2*c*x + d; // f'(x) = 4ax^3 + 3bx^2 + 2cx + d
    return y_derivative;
}
...
double fx_ddval(double a, double b, double c, double d, double e, double x)
{
	...
	y_derivative2 = 12*a*(x*x) +6*b*x + 2*c; // f''(x) = 12ax^2 + 6bx + 2c
...
double newrfind_halley(double a, double b, double c, double d, double e, double x)
{
	...
	for(trial = 0; trial <= 10000; trial++)
	{
		...
		xnew = x_n - (2*fx_val(a,b,c,d,e,x_n) * fx_dval(a,b,c,d,e,x_n))/((2*(fx_dval(a,b,c,d,e,x_n) * 
					fx_dval(a,b,c,d,e,x_n)) - (fx_val(a,b,c,d,e,x_n) * fx_ddval(a,b,c,d,e,x_n))));


		double difference = xnew - x_n;
		x_n = xnew;
		if(abs_double(difference) <= 0.000001)
		{
			break;
		}
...
int main(int argc, char **argv)
{
	...
	int bound_on_roots = rootbound(a,b,c,d,e,r,l); // Get the amount of roots within bounds.
	double CurrentRoot;
	double i;
	
	int TotalRoots = 0; // # of roots
	
	if(bound_on_roots == 0) // If there are no roots within the bounds, then state it.
	{
		printf("The polynomial has no roots in the given interval\n");
	}
	else if(bound_on_roots > 0)
	{
		for(i = l; i <= r; i= i +0.5) // Increment the left bound by 0.5 steps.
		{
			
			CurrentRoot = newrfind_halley(a,b,c,d,e,i); //Find the current root at the current step.

			printf("Root Found: %lf\n", CurrentRoot); //Print the root.
			TotalRoots = TotalRoots + 1;	//Increment the # of roots
		
		}
	
		
		if(TotalRoots == 0) // If there are no roots, then it'll say that it couldn't find any.
		{
